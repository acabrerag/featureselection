...
class FeatureSelector():
    ...
    def __init__(self, data, labels=None, random_state=None):
    ...
    def identify_missing(self, missing_threshold_min=0.2, missing_threshold_max=1):
        ...
        record_missing = pd.DataFrame(missing_series[
                                          (missing_series <= missing_threshold_min) | (
                                                  missing_series > missing_threshold_max)]).reset_index().rename(
            columns=
            {
                'index': 'feature',
                0: 'missing_fraction'})
    ...
    def identify_zero_importance(self, task, n_iterations=1, early_stopping=True, algorithm="LightGBM"):
        """
        ...
        Uses the CatBoost implementation (https://catboost.ai/docs/concepts/about.html)
        ...
        for _ in range(n_iterations):
            ...
            if algorithm == "LightGBM":
                if task == 'classification':
                    model = lgb.LGBMClassifier(n_estimators=1000, is_unbalance=True,
                                               learning_rate=0.01, verbose=-1,
                                               metric="custom", random_state=self.random_state)
                ...
                if task == 'classification':
                    model = CatBoostClassifier(learning_rate=0.1,
                                               scale_pos_weight=(len(labels) - labels.sum()) / labels.sum(), verbose=0,
                                               custom_metric=['F1:hints=skip_train~false',
                                                              'TotalF1:hints=skip_train~false'])


                elif task == 'regression':
                    model = CatBoostRegressor(n_estimators=100, learning_rate=0.05, verbose=0,
                                              random_state=self.random_state)

                else:
                    raise ValueError('Task must be either "classification" or "regression"')
            else:
                raise ValueError('Algorithm must be either "LightGBM" or "CatBoost"')
            ...
            if early_stopping:
                ...
                train_features, valid_features, train_labels, valid_labels = train_test_split(features, labels,
                                                                                              test_size=0.3,
                                                                                              stratify=labels)
                w_array = np.ones(train_labels.shape[0], dtype='float')
                for i, val in enumerate(LabelEncoder().fit_transform(train_labels)):
                    w_array[i] = class_weights[val]

                # Train the model with early stopping
                if algorithm == "CatBoost":
                    model.fit(train_features, train_labels, sample_weight=w_array,
                              eval_set=(valid_features, valid_labels),
                              early_stopping_rounds=100, verbose=0)

                else:
                    model.fit(train_features, train_labels, eval_metric=lgb_f1_score, sample_weight=w_array,
                              eval_set=(valid_features, valid_labels),
                ...
                w_array = np.ones(features.shape[0], dtype='float')
                for i, val in enumerate(LabelEncoder().fit_transform(labels)):
                    w_array[i] = class_weights[val]
                model.fit(features, labels, sample_weight=w_array)
    ...
    def identify_all(self, selection_params):
        ...
        self.identify_zero_importance(task=selection_params['task'], algorithm=selection_params['algorithm'],
